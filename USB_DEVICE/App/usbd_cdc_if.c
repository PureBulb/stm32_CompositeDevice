...
static int8_t CDC_Parse_Configuration(USBD_HandleTypeDef* pdev,USBD_CDC_ItfTypeDef* fops,uint8_t* cfgData, uint16_t length);
static int8_t CDC_Parse_Interface(USBD_HandleTypeDef* pdev,USBD_CDC_ItfTypeDef* fops,uint8_t* interfaceData,uint8_t*endpointNum);
static int8_t CDC_Parse_Endpoint(USBD_CDC_ItfTypeDef* fops,uint8_t* endpointData);
  ...
  while(seek < length){
    ...
    switch(descType){
      case USB_DESC_TYPE_INTERFACE:
        succRegFops = NULL;
        ret = CDC_Parse_Interface(pdev,fops,&cfgData[seek],&endpointNum);
...
int8_t CDC_Parse_Interface(USBD_HandleTypeDef* pdev,USBD_CDC_ItfTypeDef* fops,uint8_t* interfaceData,uint8_t*endpointNum){
  *endpointNum = 0;
  uint8_t interfaceClassType = interfaceData[5];
  if(interfaceClassType == fops->interfaceClassType){
    //Found CDC Interface
...
int8_t CDC_Parse_Endpoint(USBD_CDC_ItfTypeDef* fops,uint8_t* endpointData){
  if(NULL==fops)
    return USBD_OK;
  if(fops->endpointSize >= INTERFACE_MAX_EP_NUM)
    return USBD_FAIL;
