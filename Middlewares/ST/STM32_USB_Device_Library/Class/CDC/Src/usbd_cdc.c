...
__ALIGN_BEGIN uint8_t USBD_CDC_CfgFSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END = {
    ...
    USBD_CUSTOM_HID_REPORT_DESC_SIZE, /*wItemLength: Total length of Report
                                         descriptor*/
    0x00,
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef* pdev, uint8_t cfgidx)
{
    ...
    for (uint8_t i = 0; i < pdev->interfaceSize; i++)
    {
        ...
        {
            /* Open EP IN */
            for (uint8_t epIndex = 0; epIndex < fops->endpointSize; epIndex++)
            {
                ...
                {  // IN endpoint
                    ...
                    pdev->ep_in[epAddr & 0xFU].is_used = 1U;
                }
                else
                {  // OUT endpoint
                    USBD_LL_OpenEP(pdev, epAddr, epAttr,
                                   CDC_DATA_HS_OUT_PACKET_SIZE);
                    pdev->ep_out[epAddr & 0xFU].is_used = 1U;
                }
            }
        }
        else
    ...
    for (uint8_t i = 0; i < pdev->interfaceSize; i++)
    {
        pdev->pClassData[i] = USBD_malloc(sizeof(USBD_CDC_HandleTypeDef));
        ...
        {
            hcdc = (USBD_CDC_HandleTypeDef*)pdev->pClassData[i];

            /* Init  physical Interface components */
            ((USBD_CDC_ItfTypeDef*)pdev->pUserData[i])->Init();
            ...
            if (pdev->dev_speed == USBD_SPEED_HIGH)
            {
                ...
                for (uint8_t epIdx = 0; epIdx < fops->endpointSize; epIdx++)
                {
...
static uint8_t USBD_CDC_DeInit(USBD_HandleTypeDef* pdev, uint8_t cfgidx)
{
    ...
    for (uint8_t i = 0; i < pdev->interfaceSize; i++)
    {
        ...
        for (uint8_t epIndex = 0; epIndex < fops->endpointSize; epIndex++)
        {
            uint8_t epAddr = fops->interfaceEpAddr[epIndex];
            /* Close EP */
            USBD_LL_CloseEP(pdev, epAddr);
            if ((epAddr & 0x80U) != 0U)
            {
                pdev->ep_in[epAddr & 0xFU].is_used = 0U;
            }
            else
            {
                pdev->ep_out[epAddr & 0xFU].is_used = 0U;
            }
        }
        if (pdev->pClassData[i] != NULL)
        {
            ((USBD_CDC_ItfTypeDef*)pdev->pUserData[i])->DeInit();
            USBD_free(pdev->pClassData[i]);
...
{
    int8_t interfaceIndex = USBD_CDC_GetInterfaceIndexByEpNum(pdev, CDC_IN_EP);
    if (interfaceIndex < 0)
    {
        return USBD_FAIL;
    }
...
{
    int8_t interfaceIndex = USBD_CDC_GetInterfaceIndexByEpNum(pdev, CDC_OUT_EP);
    if (interfaceIndex < 0)
    {
        return USBD_FAIL;
    }
                                         ...
                                         uint8_t ep_addr)
{
    ...
    {
        USBD_CDC_ItfTypeDef* fops = pdev->pUserData[i];
        for (int8_t epIndex = 0; epIndex < fops->endpointSize; epIndex++)
        {
            uint8_t epAddr = fops->interfaceEpAddr[epIndex];
            if (epAddr == ep_addr)
            {
